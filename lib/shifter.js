...
module.exports = {
    ...
    **/
    ...
    internal cache mechanism. If the file is not in cache,
    ...
    **/
    _isCached: function (file, buildDir) {
        var cached = this._isFileCached(file, buildDir);

        // build.json
        if (cached && libpath.extname(file) === '.json') {
            return this._isBundleCached(file, buildDir);
        }
    ...
    /**
    Checks if the bundle defined by build.json needs to be rebuilt.

    @method _isBundleCached
    @protected
    @param {string} file The filesystem path for the file to be cached
    @param {string} buildDir The filesystem path for the build folder
    @return {boolean} `true` if the file and its content matches the internal cache, otherwise `false`.
    **/
    _isBundleCached: function (file, buildDir) {
        var json,
            ...
            i,
            filePath,
            bundlePath = libpath.dirname(file),
            bundleMetaDir = libfs.readdirSync(libpath.join(bundlePath, 'meta')),
            metaFile;

        // check meta dir
        for (i = 0; i < bundleMetaDir.length; i++) {
            metaFile = bundleMetaDir[i];
            metaFile = libpath.join(bundlePath, 'meta', metaFile);
            if (!libfs.existsSync(metaFile)) {
                debug('Unable to find ' + metaFile);
            }
            if (!this._isFileCached(metaFile, buildDir)) {
                return false;
            }
        }
        ...
        try {
            ...
            debug('Unable to parse JSON from ' + file);
            return false;
        }
        if (!json || !json.builds) {
            debug('Missing builds attribute in ' + file);
            return false;
        }

        for (modName in json.builds) {
            if (json.builds.hasOwnProperty(modName)) {
                if (!modName) {
                    continue;
                }

                modConfig = json.builds[modName];
                if (!modConfig || !modConfig.jsfiles) {
                    continue;
                }

                jsFiles = modConfig.jsfiles;
                if (!jsFiles || !jsFiles.length) {
                    continue;
                }

                // check all jsfiles defined by build.json
                for (i = 0; i < jsFiles.length; i++) {
                    filePath = libpath.join(bundlePath, 'js', jsFiles[i]);
                    if (!libfs.existsSync(filePath)) {
                        debug('Unable to find ' + filePath);
                        continue;
                    }
                    if (this._isFileCached(filePath, buildDir)) {
                        return true;
                    }
                }
            }
    ...
    @method _isJsonCached
    @protected
    @param {string} file The filesystem path for the file to be cached
    @param {string} buildDir The filesystem path for the build folder
    @return {boolean} `true` if the file and its content matches the internal cache, otherwise `false`.
    **/
    _isFileCached: function (file, buildDir) {
        var fileHash,
            data;
